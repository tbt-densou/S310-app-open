...
import com.example.frontp.model.stopBluetoothService
/**
...
fun MainScreen(navController: NavHostController, deviceViewModel: DeviceViewModel, firebaseViewModel: FirebaseViewModel) {
    val options = listOf("デバック", "全体接合", "1st走行", "2nd走行", "1stTF", "2ndTF", "3rdTF", "4thTF", "5thTF", "6thTF", "7thTF", "最終TF", "")
    ...
    val mac by deviceViewModel.macAdd.collectAsState()

    // LaunchedEffectで認証状態を確認し、未認証ならダイアログを表示
    LaunchedEffect(currentUser) {
        Log.d("アカウント","$currentUser")
        if (currentUser == null) {
            showPopup = true
        } else {
        ...
        Column(
            ...
            LaunchedEffect(key1 = "monitorConnectionStatus") {
                ...
                while (isActive) {
                    ...
                    if (currentTime - lastCheckedTime >= 2000) {
                        ...
                        deviceViewModel.isDeviceConnected(mac).let { isConnected ->
                            if (!isConnected) {
            ...
            LaunchedEffect(key1 = "connectUntilSuccess") {
                ...
                while (isActive) {
                    ...
                    if (currentTime - lastCheckedTime >= 2000) {
                        lastCheckedTime = currentTime
                        if (connectionStatus == "接続が完了しました") {
                            // サービスを開始（バックグラウンド接続の維持）
                            Log.d("MainScreen", "connectUntilSuccess mac=$mac connectionStatus=$connectionStatus")
                            startBluetoothService(context, mac)
                            break
                        }
                        deviceViewModel.onButtonClicked(mac, context)
                    }
                    delay(50)
                }
            }
            ...
            Button(onClick = {
                navController.navigate("espReader")
            }) {
                Text("受信データ表示")
            }
            ...
            Button(onClick = {
                stopBluetoothService(context, mac)
            }) {
                Text("自動接続終了")
            }
